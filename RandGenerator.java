...
public class RandGenerator {
     ...
     * @return
     */
    public String[] getNameAndGender(String gender) {
        String[] nameGender = new String[2];
        int randNum = new Random().nextInt(2) + 1;
        int strLen = gender.equals("男") ? boyName.length() : girlName.length();
        int index = (randNum & 1) == 0 ? new Random().nextInt(strLen - 1) :
                new Random().nextInt(strLen);
        nameGender[0] = gender.equals("男") ? boyName.substring(index, index + randNum) :
                girlName.substring(index, index + randNum);
        nameGender[1] = gender.equals("男") ? "男" : "女";
        return nameGender;
    }
    ...
    public String getGpa(){
        Random r = new Random();
        double randNum = Math.sqrt(0.5) * r.nextGaussian() + 2.7;
        if (randNum > 4)
            randNum = 4.0;
        DecimalFormat df = new DecimalFormat( "0.000" );
        String str=df.format(randNum);
        return str;
    }
}
